...
if [ -f ${base}/bin/push_ui.pid ] ; then
	echo "found push_ui.pid , Please run stop.sh first ,then startup.sh" 2>&2
    exit 1
fi
...
EASY_OPTS="-DappName=push_ui -Dlogback.configurationFile=$logback_configurationFile -Deasy.conf=$easy_conf"
