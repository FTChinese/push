...
if [ -f ${base}/bin/update_token.pid ] ; then
	echo "found update_token.pid , Please run stop.sh first ,then startup.sh" 2>&2
    exit 1
fi
...
EASY_OPTS="-DappName=update_token -Dlogback.configurationFile=$logback_configurationFile -Deasy.conf=$easy_conf"
